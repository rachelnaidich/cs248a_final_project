...
class OcclusionRenderer {
    ...
    init(session: ARSession, metalDevice device: MTLDevice, renderDestination: RenderDestinationProvider, handType: Int) {
    ...
    func loadMetal() {
        ...
        var compositeImageFragmentFunction: MTLFunction
        if (handType == 0) {
            compositeImageFragmentFunction = defaultLibrary.makeFunction(name: "compositeImageFragmentShader0")!
        } else if (handType == 1) {
            compositeImageFragmentFunction = defaultLibrary.makeFunction(name: "compositeImageFragmentShader1")!
        } else if (handType == 2) {
            compositeImageFragmentFunction = defaultLibrary.makeFunction(name: "compositeImageFragmentShader2")!
        } else {
            compositeImageFragmentFunction = defaultLibrary.makeFunction(name: "compositeImageFragmentShader3")!
        }
    ...
    func loadAssets() {
        ...
        let mesh = MDLMesh(boxWithExtent: vector3(0.075, 0.075, 0.075), segments: vector3(1, 1, 1), inwardNormals: false,
