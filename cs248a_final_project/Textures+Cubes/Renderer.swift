...
class Renderer {
    ...
    func loadMetal() {
        ...
        debugUniformBuffer.label = "DebugUniformBuffer"
    ...
    func loadAssets() {
        ...
        var mesh = MDLMesh(boxWithExtent: vector3(0.075, 0.075, 0.075), segments: vector3(1, 1, 1), inwardNormals: false, geometryType: .triangles, allocator: metalAllocator)
        ...
        mesh = MDLMesh(planeWithExtent: vector3(0.1, 0.1, 0.1), segments: vector2(1, 1), geometryType: .triangles, allocator: metalAllocator)
        mesh.vertexDescriptor = vertexDescriptor
        do { try debugMesh = MTKMesh(mesh: mesh, device: device)
        } catch let error { print(error) }
    }
    ...
    func updateBufferStates() {
        ...
        debugUniformBufferOffset = kAlignedInstanceUniformsSize * uniformBufferIndex
    ...
    func updateAnchors(frame: ARFrame) {
        ...
        debugInstanceCount = min(count, kMaxAnchorInstanceCount - (anchorInstanceCount - count))
...
//            .flatMap({ $0 as? UIWindowScene })?.interfaceOrientation else {
//            return
//        }
    ...
    func drawAnchorGeometry(renderEncoder: MTLRenderCommandEncoder) {
        guard anchorInstanceCount - debugInstanceCount > 0 else {
            return
        }
