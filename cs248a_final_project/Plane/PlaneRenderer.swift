...
class PlaneRenderer {
    ...
    var planePipelineState:      MTLRenderPipelineState!
    var planeDepthState:         MTLDepthStencilState!
    ...
    init(session: ARSession, metalDevice device: MTLDevice, renderDestination: RenderDestinationProvider) {
        ...
        do {
            planeTexture = try textureLoader.newTexture( name: "texture02", scaleFactor: 1.0, bundle: Bundle.main, options: nil )
        } catch {
    ...
    func update() {
        ...
        if let commandBuffer = commandQueue.makeCommandBuffer() {
            ...
            if let renderPassDescriptor = renderDestination.currentRenderPassDescriptor, let currentDrawable = renderDestination.currentDrawable {
                ...
                let oldAction = renderPassDescriptor.colorAttachments[0].loadAction
                ...
                if let planeEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: renderPassDescriptor) {
                    
                    planeEncoder.label = "MyPlaneEncoder"
                    
                    drawPlaneGeometry(renderEncoder: planeEncoder)
                    
                    // We're done encoding commands
                    planeEncoder.endEncoding()
                }
                
                renderPassDescriptor.colorAttachments[0].loadAction = oldAction
    ...
    func loadMetal() {
        ...
        planeVertexBuffer = device.makeBuffer(bytes: planeVertexData, length: planeVertexData.count * MemoryLayout<Float>.stride, options: [] )
        planeIndexBuffer  = device.makeBuffer(bytes: planeIndexData,  length: planeIndexData.count * MemoryLayout<Int32>.stride, options: [] )
        ...
        // Plane Stuff
        let planeVertexFunction   = defaultLibrary.makeFunction(name: "planeVertex")!
        let planeFragmentFunction = defaultLibrary.makeFunction(name: "planeFragment")!
        
        // Create a vertex descriptor for our image plane vertex buffer
        let planeVertexDescriptor = MTLVertexDescriptor()
        
        // Positions.
        planeVertexDescriptor.attributes[0].format = .float4
        planeVertexDescriptor.attributes[0].offset = 0
        planeVertexDescriptor.attributes[0].bufferIndex = 0
        
        // Texture coordinates.
        planeVertexDescriptor.attributes[1].format = .float2
        planeVertexDescriptor.attributes[1].offset = 16
        planeVertexDescriptor.attributes[1].bufferIndex = 0
        
        // Buffer Layout
        planeVertexDescriptor.layouts[0].stride = 24
        planeVertexDescriptor.layouts[0].stepRate = 1
        planeVertexDescriptor.layouts[0].stepFunction = .perVertex
        
        // Create a pipeline state for rendering the captured image
        let planePipelineDescriptor = MTLRenderPipelineDescriptor()
        planePipelineDescriptor.vertexFunction                  = planeVertexFunction
        planePipelineDescriptor.fragmentFunction                = planeFragmentFunction
        planePipelineDescriptor.vertexDescriptor                = planeVertexDescriptor
        planePipelineDescriptor.colorAttachments[0].pixelFormat = renderDestination.colorPixelFormat
        planePipelineDescriptor.depthAttachmentPixelFormat      = renderDestination.depthStencilPixelFormat
        planePipelineDescriptor.stencilAttachmentPixelFormat    = renderDestination.depthStencilPixelFormat
        
        do {
            try planePipelineState = device.makeRenderPipelineState(descriptor: planePipelineDescriptor)
        } catch let error {
            print("Failed to create pipeline state, error \(error)")
        }
        
        let planeDepthStateDescriptor = MTLDepthStencilDescriptor()
        planeDepthStateDescriptor.depthCompareFunction = .less
        planeDepthStateDescriptor.isDepthWriteEnabled = true
        planeDepthState = device.makeDepthStencilState(descriptor: planeDepthStateDescriptor)
    ...
    func updateSharedUniforms(frame: ARFrame) {
        ...
        self.projectionMatrix = frame.camera.projectionMatrix(for: .portrait, viewportSize: viewportSize, zNear: 0.001, zFar: 1000)
        var coordinateSpaceTransform = matrix_identity_float4x4
        coordinateSpaceTransform.columns.2.z = -1.0
        self.viewMatrix = frame.camera.viewMatrix(for: .portrait) * coordinateSpaceTransform
...
//        }
        ...
        for anchor in frame.anchors {
            ...
            if let planeAnchor = anchor as? ARPlaneAnchor {
                var com = planeAnchor.transform.columns.3 + SIMD4<Float>( planeAnchor.center, 1.0 )
                com.z *= -1.0
                coms.append( com )
            }
        }
    ...
    func drawAnchorGeometry(renderEncoder: MTLRenderCommandEncoder) {
        ...
        renderEncoder.setViewport( MTLViewport( originX: 0.0, originY: 0.0, width: screenSize.width, height: screenSize.height, znear: 0.0, zfar: 1.0 ))
    ...
    func drawPlaneGeometry(renderEncoder: MTLRenderCommandEncoder) {
        print(coms.count)
        guard coms.count > 0 else {
            return
        }
        
        // Push a debug group allowing us to identify render commands in the GPU Frame Capture tool
        renderEncoder.pushDebugGroup("DrawPlanes")
        
        // Set render command encoder state
        renderEncoder.setCullMode(.back)
        renderEncoder.setRenderPipelineState(planePipelineState)
        renderEncoder.setDepthStencilState(planeDepthState)
        
        renderEncoder.setVertexBuffer( planeVertexBuffer, offset: 0, index: 0 )
        var planeWidthFloat = 1.0
        renderEncoder.setVertexBytes( &planeWidthFloat,  length: MemoryLayout<Float>.stride,                     index: 1 )
        renderEncoder.setVertexBytes( &coms,             length: MemoryLayout<SIMD4<Float>>.stride * coms.count, index: 2 )
        renderEncoder.setVertexBytes( &viewMatrix,       length: MemoryLayout<float4x4>.stride,                  index: 3 )
        renderEncoder.setVertexBytes( &projectionMatrix, length: MemoryLayout<float4x4>.stride,                  index: 4 )

        renderEncoder.setFragmentTexture( planeTexture, index: 0 )
